...
# CSS стили остаются без изменений, так как они не затрагивают логику
CSS_STYLE = """
...
def init_session_state():
    defaults = {
        ...
        'show_all_days': False  # <-- ИЗМЕНЕНИЕ: Добавлено состояние для отображения всех дней
    }
...
# --- ФУНКЦИИ ---
# <-- ИЗМЕНЕНИЕ: Функция для сброса отображения дней при смене дат
def reset_days_view():
...
with st.container():
    ...
    with col1:
        start_date = st.date_input(
            ...
            on_change=reset_days_view  # <-- ИЗМЕНЕНИЕ: Сброс при смене даты
        )
    with col2:
...
# ... (Код доходов и расходов остается без изменений)
with col1:
...
if metrics and metrics['balance'] >= 0:
    ...
    with st.expander("💸 Быстрый ввод расхода на сегодня", expanded=False):
        # ... (Код быстрого ввода без изменений)
    ...
    with st.container():
        ...
        # <-- ИЗМЕНЕНИЕ: Логика для отображения всех дней
        ...
        # <-- ИЗМЕНЕНИЕ: Кнопка для отображения всех дней
