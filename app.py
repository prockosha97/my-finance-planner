...
if st.session_state["authentication_status"]:
    ...
    if metrics and metrics.get('balance', -1) >= 0:
        ...
        with st.container():
            ...
            if st.session_state.get('show_all_days', False):
            ...
            for i in range(display_days):
                ...
                daily_budget = (disposable_income / days_in_period if days_in_period > 0 else 0) + rollover
                day_spends = st.session_state.daily_spends.get(day_key, [])
                total_day_spend = sum(item['amount'] for item in day_spends)
                day_balance = daily_budget - total_day_spend
                ...
                with st.container():
                    ...
                    with row_cols[1]:
                        st.markdown(f"`{format_currency(daily_budget)} ₽`")
                    with row_cols[2]:
                    ...
                    st.markdown('<hr style="margin: 0.5rem 0; border-color: var(--border-light);">', unsafe_allow_html=True)
            if not st.session_state.get('show_all_days', False) and days_in_period > display_days:
    ...
    if metrics and metrics.get('balance', -1) >= 0:
        ...
        with col_export:
            # Пересчитываем значения для отчета, чтобы они были доступны
            disposable_income = metrics.get('disposable_income', 0)
            daily_budget = disposable_income / days_in_period if days_in_period > 0 else 0
            report_text = f"""ФИНАНСОВЫЙ ОТЧЕТ
==================
Период: {start_date.strftime('%d.%m.%Y')} - {end_date.strftime('%d.%m.%Y')}
Дней в периоде: {days_in_period}
ДОХОДЫ: {format_currency(metrics.get('total_income', 0))} ₽
РАСХОДЫ: {format_currency(metrics.get('total_expenses', 0))} ₽
НАКОПЛЕНИЯ: {metrics.get('savings_percentage', 0)}% ({format_currency(metrics.get('savings_amount', 0))} ₽)
